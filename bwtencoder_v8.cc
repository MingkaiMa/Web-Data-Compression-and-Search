...
void coreFunc(char delimiter,  char* indexFolder, char* sour_file, char* dest_file){
	...
	map<int, int> delimiter_pos_table;

	for(int i = 0; i < fileSize; ++i){
		if(readBuffer[i] == delimiter){
			++delimiterCount;
			delimiter_pos_table[i] = 0;
		}
	}
	...
	int firstPartSize = alignment * 2 + delimiterCount * (alignment * 2);
	...
	int leftSize = fileSize - firstPartSize;
	...
	if(rowNb == 0){
		...
		for(int i = 0; i < encoder.n; ++i){
			if(encoder.sa[i] == 0){
				...
				if(delimiter_pos_table.count(encoder.sa[i] - 1) > 0){
					delimiter_pos_table[encoder.sa[i] - 1] = i;
				}
		...
		writeBufferFunc(delimiterBuffer, delimiterBufferIndex, lastIndex, alignment);
		writeBufferFunc(delimiterBuffer, delimiterBufferIndex, delimiterCount, alignment);

		for(auto it = delimiter_pos_table.begin(); it != delimiter_pos_table.end(); ++it){
			writeBufferFunc(delimiterBuffer, delimiterBufferIndex, it->first, alignment);
			writeBufferFunc(delimiterBuffer, delimiterBufferIndex, it->second, alignment);
		}
	...
	writeBuffer = new unsigned char[rowSize];
	int writeBufferIndex = 0;
