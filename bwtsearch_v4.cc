...
void writeIndexFile(int* count, string index_file, int alignment, int writeSize, int rowNb){
...
int getNextPosition(string index_file, string position_file, int position, int alignment, int writeSize, int* C, char* bwt_encoded_file, int lastIndex){
	...
	// cout << "position:-- " << position << "  -- lastIndex: " << lastIndex << "\n";
...
int getPosition(string position_file, string index_file, int position, int alignment, int writeSize, int* C, char* bwt_encoded_file, int lastIndex){
	...
	fromFile.seekg(deliNum * alignment, ios::cur);
	...
	while(true){
		...
		if(position > beginNb && (position - beginNb) % step == 0 && position <= lastNb){
			...
			fromFile.seekg(2 * alignment + deliNum * alignment + (movebit) * (2 * alignment) + alignment , ios::beg);
			delimiterNumBuffer = new unsigned char[alignment];
			fromFile.read((char*)(&delimiterNumBuffer[0]), alignment);
			int saValue = 0;
			for(int i = 0; i < alignment; ++i){
...
void searchWithWrite(char delimiter, char* bwt_encoded_file, char* index_folder, char* mode, char* queryString, int* C, int writeSize, int alignment, int lastIndex){
		...
		for(int i = 0; i < readBlockSize; ++i){
			...
			if(inIndex == lastIndex){
				...
				if((inIndex + 1) % writeSize == 0){
					//write here
					writeIndexFile(count, index_file, alignment, writeSize, inIndex);
				}
				++inIndex;
				i -= 1;
				continue;
...
int main(int argc, char *argv[]){
	...
	if(oneRowSize <= fileSize){
		...
		if(!file_exist(index_file))
			searchWithWrite(delimiter, bwt_encoded_file, index_folder, mode, queryString, C, oneRowSize, alignment, lastIndex);
		else
