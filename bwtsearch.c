...
#define bwtFileLimit 7000000


int file_exist(char* index_file){
...
void writeBufferFunc(unsigned char* writeBuffer, int* writeIndex, int toWrite, int alignment){
	...
	// int writeIndex = *writeIndex_;

	if(alignment == 1){
		writeBuffer[(*writeIndex)] = (toWrite);
		++(*writeIndex);
	}
	else if(alignment == 2){
		writeBuffer[(*writeIndex)] = (toWrite >> 8);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = (toWrite & 255);
		++(*writeIndex);
	}
	else if(alignment == 3){
		writeBuffer[(*writeIndex)] = (toWrite >> 16);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = ((toWrite >> 8) & 255);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = (toWrite & 255);
		++(*writeIndex);
	}
	else if(alignment == 4){
		writeBuffer[(*writeIndex)] = (toWrite >> 24);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = ((toWrite >> 16) & 255);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = ((toWrite >> 8) & 255);
		++(*writeIndex);
		writeBuffer[(*writeIndex)] = (toWrite & 255);
...
void writeIndexFile(int* count, char* index_file, int alignment, int writeSize, int rowNb, int* charList, int charNb){

	// unsigned char* writeBuffer = new unsigned char[writeSize];
	unsigned char* writeBuffer = (unsigned char*) malloc(sizeof(unsigned char) * writeSize);
	int writeIndex = 0;
	...
	FILE* toFile;
	toFile = fopen(index_file, "ab");
	fwrite(writeBuffer, 1, writeIndex, toFile);
	fclose(toFile);
}


int getOCC(char c, int position, char* index_file, int alignment, int writeSize, char* bwt_encoded_file, int lastIndex, int* getOCC_aid){

	// ifstream fromFile(index_file, ios::binary);
	// ifstream fromFile2(bwt_encoded_file, ios::binary);
	// fromFile2.seekg(0, ios::beg);

	FILE* fromFile, *fromFile2;

	fromFile = fopen(index_file, "rb");
	fromFile2 = fopen(bwt_encoded_file, "rb");
	...
	if(!file_exist(index_file)){
		...
		if((position >= lastIndex && (position - 1) < writeSize - 1)){
			...
			char* readBuffer = (char* )malloc(sizeof(char) * (position + 1));

			// fromFile2.read(readBuffer, position + 1);
			fread(readBuffer, 1, position + 1, fromFile2);
	...
	fseek(fromFile, readIndex, SEEK_SET);


	unsigned char* getBuffer =  (unsigned char*) malloc(alignment * sizeof(unsigned char));
	// fromFile.read((char*)(&getBuffer[0]), alignment);
	fread(getBuffer, 1, alignment, fromFile);
	...
	int toMoveIndex = getOCC_aid[c - 0];

	// fromFile.seekg(toMoveIndex * alignment, ios::cur);

	fseek(fromFile, toMoveIndex * alignment, SEEK_CUR);
	...
	char* locateBuffer = (char* ) malloc(sizeof(char) * readSize);
	...
	fseek(fromFile2, rowNb, SEEK_SET);


	// fromFile2.read(locateBuffer, readSize);
	fread(locateBuffer, 1, readSize, fromFile2);
...
int getOCC3(char c, int position, char* index_file, int alignment, int writeSize, char* bwt_encoded_file, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer, FILE* fromFile, FILE* fromFile2){
	...
	unsigned char* getBuffer = (unsigned char*) malloc(writeSize * sizeof(unsigned char));

	// fromFile.read((char*)(&getBuffer[0]), writeSize);
	fread(getBuffer, 1, writeSize, fromFile);
...
int getNextPosition3(char* index_file, char* position_file, char c, int position, int alignment, int writeSize, int* C, char* bwt_encoded_file, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer, FILE* indexFile, FILE* bwtFile){

	// printf("come in next\n");
	if(position == lastIndex){
		char c = 0;
		return C[0] + getOCC3(c, position, index_file, alignment, writeSize, bwt_encoded_file, lastIndex, getOCC_aid, bwtBuffer, indexFile, bwtFile) - 1;
	}

	// printf("should here?\n");
	return C[c - 0] + getOCC3(c, position, index_file, alignment, writeSize, bwt_encoded_file, lastIndex, getOCC_aid, bwtBuffer, indexFile, bwtFile) - 1;
}


int getNextPosition_NA(FILE* indexFile, FILE* bwtFile, int position, int alignment, int writeSize, int* C, char* index_file, char* bwt_encoded_file, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer){
	...
	if(position < lastIndex){
		...
		fseek(bwtFile, position, SEEK_SET);
		fread(&c, 1, 1, bwtFile);
	...
	if(position > lastIndex){
		...
		fseek(bwtFile, position - 1, SEEK_SET);
		fread(&c, 1, 1, bwtFile);
...
//
int getSa_v1(char* index_file, char* bwt_encoded_file, FILE* indexFile, FILE* bwtFile, int position, int alignment, int writeSize, int* C, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer){
...
//
int getSa_v2(char* index_file, char* bwt_encoded_file, int beginNb, int beginVal, FILE* indexFile, FILE* bwtFile, int position, int alignment, int writeSize, int* C, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer){
...
int getSa_v3(char* index_file, char* bwt_encoded_file, int beginNb, int beginVal, int lastNb, int lastVal, int step, FILE* posFile, FILE* indexFile, FILE* bwtFile, int position, int alignment, int writeSize, int* C, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer, int deliNum){
	...
	while(1){
		...
		if(position > beginNb && (position - beginNb) % step == 0 && position <= lastNb){
			...
			fseek(posFile,   2 * alignment + deliNum * (alignment * 2) + (movebit) * (2 * alignment) + alignment, SEEK_SET);

			// unsigned char* delimiterNumBuffer = new unsigned char[alignment];
			unsigned char* delimiterNumBuffer = (unsigned char*) malloc(sizeof(unsigned char) * alignment);

			// posFile.read((char*)(&delimiterNumBuffer[0]), alignment);
			fread(delimiterNumBuffer, 1, alignment, posFile);
...
void searchMode_M(char* index_file, int alignment, int writeSize, const char* queryString, int* C, char* bwt_encoded_file, int lastIndex, int* getOCC_aid){
	...
	while(first <= last && i >= 1){
		...
		first = C[c - 0] + getOCC(c, first - 1, index_file, alignment, writeSize, bwt_encoded_file, lastIndex, getOCC_aid);

		last =  C[c - 0] + getOCC(c, last,      index_file, alignment, writeSize, bwt_encoded_file, lastIndex, getOCC_aid) - 1;
...
int own_lower_bound(int* A, int n, int val){
	int left = 0, right = n;
	int mid;
	while(left < right){
		mid = (left + right) >> 1;
...
int own_upper_bound(int* A, int n, int val){
	int left = 0, right = n;
	int mid;
	while(left < right){
		mid = (left + right) >> 1;
...
void allocate(int* A, int A_Len, int alignment, FILE* posFile, char* flagBits, int deliNum){

	// cout << "come in\n";
	// sort(A, A + A_Len);
	qsort(A, A_Len, sizeof(int), compareFunc);
	int block = 2 * alignment * 250000;
	int index = 0;
	int curNb;
	int preNumber = 0;


	// posFile.seekg(2 * alignment, ios::beg);

	fseek(posFile, 2 * alignment, SEEK_SET);

	//
	//	read all into memory
	//

	unsigned char* deliNb_pos;


	if(deliNum * (alignment * 2) <= block){


		// unsigned char* deliNb_pos = new unsigned char[deliNum * (alignment * 2)];

		deliNb_pos = (unsigned char*) malloc(sizeof(unsigned char) * deliNum * (alignment * 2));

		// posFile.read((char*)(&deliNb_pos[0]), deliNum * (alignment * 2));

		fread(deliNb_pos, 1, deliNum * alignment * 2, posFile);

		// int* deliNb_list = new int[deliNum];
		int* deliNb_list = (int*) malloc(sizeof(int) * deliNum);
		...
		for(int i = 0; i < A_Len; ++i){
			...
			// auto low = lower_bound(deliNb_list, deliNb_list + deliNum, A[i]);
			int low = own_lower_bound(deliNb_list, deliNum, A[i]);
			flagBits[low] = '1';
		}
	...
	// unsigned char* buffer = new unsigned char[block];
	unsigned char* buffer = (unsigned char*) malloc(sizeof(unsigned char) * block);


	int leftSize = deliNum * (alignment * 2);

	int returnFlag = 0;

	while(1){

		// posFile.read((char*)(&buffer[0]), block);
		fread(buffer, 1, block, posFile);

		for(int i = 0; i < block; i += 2 * alignment){
			curNb = 0;

			for(int j = i; j < i + alignment; ++j){
				curNb = curNb << 8;
				curNb = curNb | buffer[j];		
			}

			++index;

			int upper = own_upper_bound(A, A_Len, preNumber);
			int low = own_lower_bound(A, A_Len, curNb);
		...
		if(leftSize > block)
			...
			block = leftSize;
			// buffer = new unsigned char[block];
...
void searchMode_N(char* position_file, char* index_file, int alignment, int writeSize, const char* queryString, int* C, char* bwt_encoded_file, char* mode, int lastIndex, int* getOCC_aid){

	// ifstream bwtFile(bwt_encoded_file, ios::binary);
	FILE* bwtFile;
	bwtFile = fopen(bwt_encoded_file, "rb");

	// bwtFile.seekg(0, ios::end);
	fseek(bwtFile, 0, SEEK_END);

	// int bwtFileSize = bwtFile.tellg();
	int bwtFileSize = ftell(bwtFile);

	// bwtFile.seekg(0, ios::beg);
	fseek(bwtFile, 0, SEEK_SET);


	int bwtBlock = 7000000;
	unsigned char* bwtBuffer;

	if(bwtFileSize < bwtBlock){

		// bwtBuffer = new unsigned char[bwtFileSize];
		bwtBuffer = (unsigned char*) malloc(sizeof(unsigned char)* bwtFileSize);

		// bwtFile.read((char*)(&bwtBuffer[0]), bwtFileSize);
		fread(bwtBuffer, 1, bwtFileSize, bwtFile);

	}
	else{

		// bwtBuffer = new unsigned char[bwtBlock];
		bwtBuffer = (unsigned char*) malloc(sizeof(unsigned char) * bwtBlock);

		// bwtFile.read((char*)(&bwtBuffer[0]), bwtBlock);
		fread(bwtBuffer, 1, bwtBlock, bwtFile);
	}

	// ifstream indexFile(index_file, ios::binary);

	FILE* indexFile;
	indexFile = fopen(index_file, "rb");
	...
	FILE* fromFile;
	fromFile = fopen(position_file, "rb");

	// unsigned char* delimiterNumBuffer = new unsigned char[alignment];
	unsigned char* delimiterNumBuffer = (unsigned char*) malloc(sizeof(unsigned char) * alignment);
	// fromFile.seekg(alignment, ios::beg);
	fseek(fromFile, alignment, SEEK_SET);

	// fromFile.read((char*)(&delimiterNumBuffer[0]), alignment);
	fread(delimiterNumBuffer, 1, alignment, fromFile);
	...
	// fromFile.seekg(deliNum * (alignment * 2), ios::cur);
	fseek(fromFile, deliNum * (alignment * 2), SEEK_CUR);
	...
	if(fgetc(fromFile) == EOF){

		// char* flagBits = new char[deliNum];
		char* flagBits = (char*) malloc(sizeof(char) * deliNum);

		memset(flagBits, '0', deliNum);

		// int* A = new int[6001];
		int* A = (int*) malloc(sizeof(int) * 5001);
		int aIndex = 0;
		...
		allocate(A, aIndex, alignment, fromFile, flagBits, deliNum);
	...
	fseek(fromFile, -1, SEEK_CUR);


	// delimiterNumBuffer = new unsigned char[alignment];
	delimiterNumBuffer = (unsigned char*) malloc(sizeof(unsigned char) * alignment);
	...
	int beginEndIndex = ftell(fromFile);
	...
	fseek(fromFile, -2 * alignment, SEEK_END);
	...
	int lastEndIndex = ftell(fromFile);

	// printf("lastEndIndex: %d\n", lastEndIndex);


	// printf("lastNb: %d,  beginNb: %d, beginEndIndex: %d, alignment: %d\n", lastNb, beginNb, beginEndIndex, alignment);
	...
	//allocate(int* A, int A_Len, int alignment, ifstream& posFile, char* flagBits, int deliNum
	allocate(A, aIndex, alignment, fromFile, flagBits, deliNum);
...
void recoverRecord(char delimiter, char* position_file, char* index_file, int* C, int writeSize, int alignment, int position, char* bwt_encoded_file, int lastIndex, int* getOCC_aid, unsigned char* bwtBuffer){
	...
	char* res = (char*) malloc(sizeof(char) * 5001);
	int resIndex = 0;
	...
	FILE* fromFile;
	fromFile = fopen(bwt_encoded_file, "rb");
	...
	if(nextPosition > lastIndex){
		...
		if(tempnextPosition < bwtFileLimit){
			...
			// fromFile.seekg(tempnextPosition, ios::beg);
			fseek(fromFile, tempnextPosition, SEEK_SET);
		...
		if(nextPosition < bwtFileLimit){
			...
			// fromFile.seekg(nextPosition, ios::beg);
			fseek(fromFile, nextPosition, SEEK_SET);
...
void searchMode_I(char delimiter, char* bwt_encoded_file, char* position_file, char* index_file, int* C, int writeSize, int alignment, int startRec, int endRec, int fileSize, int lastIndex, int* getOCC_aid){


	// cout << "come in\n";
	// ifstream bwtFile(bwt_encoded_file, ios::binary);
	FILE* bwtFile = fopen(bwt_encoded_file, "rb");
	...
	// ifstream fromFile(position_file, ios::binary);
	FILE* fromFile = fopen(position_file, "rb");
	...
	fseek(fromFile, 2 * alignment * startRec, SEEK_SET);


	// unsigned char* deliNb_pos = new unsigned char[(endRec - startRec + 1) * 2 * alignment];

	unsigned char* deliNb_pos = (unsigned char*) malloc(sizeof(unsigned char) * (endRec - startRec + 1) * 2 * alignment);


	// fromFile.read((char*)(&deliNb_pos[0]), (endRec - startRec + 1) * 2 * alignment);
	fread(deliNb_pos, 1, (endRec - startRec + 1) * 2 * alignment, fromFile);

	// int* deli_pos_list = new int[endRec - startRec + 1];
	int* deli_pos_list = (int*) malloc(sizeof(int) * (endRec - startRec + 1));
...
void searchWithWrite(char delimiter, char* bwt_encoded_file, char* index_folder, char* mode, char* queryString, int* C, int writeSize, int alignment, int lastIndex, int* charList, int charNb, int* getOCC_aid){
	...
	int fileSize;
	// ifstream fromFile(bwt_encoded_file, ios::binary);
	FILE* fromFile = fopen(bwt_encoded_file, "rb");
	...
	char* readBuffer = (char*) malloc(sizeof(char) * readBlockSize);


	// ///////////////////////////////////////
	char* searchIndex = "searchIndexFile.aux";
	char* posiFile_char = "positionFile.aux";

	int len1 = strlen(index_folder);
	int len2 = strlen(bwt_encoded_file);
	int len3 = strlen(searchIndex);
	int len4 = strlen(posiFile_char);

	char* index_file = (char*) malloc(sizeof(char)* (len1 + 1 + len2 + len3));
	char* position_file = (char*) malloc(sizeof(char) * (len2 + len4));

	memcpy(index_file, index_folder, len1);
	memcpy(index_file + len1, "/", 1);
	memcpy(index_file + len1 + 1, bwt_encoded_file, len2);
	memcpy(index_file + len1 + 1 + len2, searchIndex, len3 + 1);


	memcpy(position_file, bwt_encoded_file, len2);
	memcpy(position_file + len2, posiFile_char, len4 + 1);
	...
	// printf("%s\n%s\n", index_file, position_file);

	size_t nread;


	// while(fromFile.read(readBuffer, readBlockSize)){
	while((nread = fread(readBuffer, 1, readBlockSize, fromFile)) > 0){
...
void searchWithoutWriteWithRead(char delimiter, char* bwt_encoded_file, char* index_folder, char* mode, char* queryString, int* C, int writeSize, int alignment, int lastIndex, int* getOCC_aid){
	...
	if(strcmp(mode, "-m") == 0){
		...
		// printf("then here\n");
		// printf("%d----%d\n", startRec, endRec);
		// cout << "fileSize: endhere" << fileSize << "\n";
		searchMode_I(delimiter, bwt_encoded_file, position_file, index_file, C, writeSize, alignment, startRec, endRec, fileSize, lastIndex, getOCC_aid);
	}
	return;
}
...
int main(int argc, char *argv[]){
	...
	unsigned char* lastIndexBuffer = (unsigned char*) malloc(sizeof(unsigned char) * alignment);

	// ifstream tempFile(position_file, ios::binary);
	FILE* tempFile = fopen(position_file, "rb");

	// tempFile.read((char*)(&lastIndexBuffer[0]), alignment);
	fread(lastIndexBuffer, 1, alignment, tempFile);
	...
	free(lastIndexBuffer);



	int readBlockSize = 9000000;
	...
	int* charList = (int*) malloc(sizeof(int) * charNb);
	...
	int* getOCC_aid = (int*) malloc(sizeof(int) * 128);
